...
- `assets/feature-template/field-alignment.md`：字段级对齐关卡模板
...
- `assets/feature-template/design/ui-review.md`：UI 走查与可执行修正建议模板
...
先归档到 `design/feedback.md`，再由 `frontend-agent` 快修并写入 `frontend/review-fixes.md`。这类连续小反馈默认保持当前阶段，不逐条推进 `status.yaml`；等使用者确认本轮反馈收口后，再统一进入下一步。如果反馈来自 QA、UAT、送测、线上回归或缺陷平台链接，改走 `bugs/`。
