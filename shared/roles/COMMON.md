...
开发期使用者连续提出的小反馈，默认属于当前阶段内修正，不是新的流程阶段。

- 适用范围：开发中即时反馈、UI 微调、文案/间距/层级/对齐等小修正，且不改变需求范围、接口契约、测试门禁或主流程
- 记录方式：追加 `activity.md`、`design/feedback.md` 或 `frontend/review-fixes.md`
- 状态处理：默认不修改 `status.phase`、`status.next`，也不要求每个小点都追加 `status.history`
- 收口方式：当使用者明确说“这一轮反馈完成 / 收口 / 继续推进 / 可以进入下一步”时，再统一检查门禁并推进 `status.yaml`
- 例外：如果反馈来自 QA、UAT、送测、线上回归、缺陷平台，或影响产品范围、接口、数据、权限、验收标准，必须退出阶段内修正，进入正式 Bug、澄清或阻塞流程

## 分层验证档位

前端、UI、接口联调或测试相关任务必须按变更类型选择验证档位，并在对应产物中记录已执行、未执行和原因。

- 基础必跑：类型检查或构建期检查，以及与本次改动相关的定向 lint；如果项目没有对应命令，记录不存在或不可执行原因
- 建议跑：本地 dev 页面或预览页面可达性检查，至少记录目标页面 URL、HTTP 状态或页面可打开结论
- UI 变更必跑：浏览器截图、人工截图核对或等价的视觉证据；Figma 驱动页面不能只用 type-check、lint 或单测作为通过依据
- 接口行为变更必跑：浏览器 Network、抓包、日志或测试工具核对请求数量、请求路径、方法、关键参数和错误路径；只改样式且不触发接口时记录“不适用”

如果环境、账号、数据、权限或工具限制导致某档位无法执行，必须写明阻塞原因、风险和需要使用者补充什么；不得把未执行写成通过。
