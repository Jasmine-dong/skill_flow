...
- 截图/手工验证：UI 变更必须记录检查页面、视口、浏览器和截图来源或人工核对结论；无法验证时写明原因
- Network 核对：接口行为变更必须记录请求数量、路径、方法、关键参数和错误路径核对结果；无接口行为变化时记录“不适用”
...
- `frontend/integration.md` 已写明实现、分层验证结果、建议测试点、影响范围和扩测建议
- `frontend/integration.md` 已写明 UI 关键项自检结果；未执行或未覆盖的项必须说明原因和风险
- UI 变更必须有浏览器截图或人工截图核对证据；接口行为变更必须有 Network 请求数量/路径核对证据；缺失时不得直接推进，除非已记录阻塞或使用者确认接受风险
...
- 不用类型检查、lint 或单测通过替代 UI 关键项自检、浏览器截图核对或 Network 请求核对
- 不把送测后缺陷伪装成开发期 UI 快修
